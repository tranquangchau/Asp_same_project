...
namespace ImageBrowser
{
	...
	public class ImageTools
	{
		...
		/// ie. /pics
		/// </summary>
		public static string VirtualDirectory = System.Configuration.ConfigurationSettings.AppSettings["ImageVirtualDir"];

		/// <summary>
		/// Where the virtual directory maps to on disk
		/// </summary>
        //public static string RootDirectory = HttpContext.Current.Server.MapPath(VirtualDirectory);
        public static string RootDirectory = System.Web.HttpContext.Current.Server.MapPath("~"+VirtualDirectory);
		...
		{
			return new SubDirectoryWrapper(dir, HttpContext.Current.Request.ApplicationPath + "/folder.jpg");
		}
		...
		public static string GetPath(string queryPath)
		{
			...
			if ( path.StartsWith(VirtualDirectory) ) path = path.Substring(VirtualDirectory.Length ,path.Length - VirtualDirectory.Length );
		...
		public static string CreateImage( string src, string dest, int width )
		{
			...
			try
			{
				if ( File.Exists(RootDirectory + "/" + dest) ) return description;

				string path = Directory.GetParent(RootDirectory + "/" + dest).FullName;
